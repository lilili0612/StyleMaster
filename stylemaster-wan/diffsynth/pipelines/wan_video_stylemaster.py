...
class WanVideoStyleMasterPipeline(BasePipeline):
    ...
    def from_model_manager(model_manager: ModelManager, torch_dtype=None, device=None):
        ...
        pipe = WanVideoStyleMasterPipeline(device=device, torch_dtype=torch_dtype)
    ...
    def __call__(
        ...
        image_embeds_ps.image_embeds = image_embeds_ps.image_embeds.to(dtype=self.torch_dtype, device=self.device)
        image_embeds.last_hidden_state = image_embeds.last_hidden_state.to(dtype=self.torch_dtype, device=self.device)
        style_feat = self.dit.style_model.project_image_embeddings(image_embeds,
                                                                   image_embeds_ps,
                                                                   prompt_emb_posi["context"].squeeze(1).to(dtype=self.torch_dtype, device=self.device))

        style_feat = style_feat.to(dtype=self.torch_dtype, device=self.device)
